...
static esp_err_t get_state_handler(httpd_req_t *req)
{
    ...
    snprintf(response_buffer, sizeof(response_buffer),
             "{\"status\":\"%s\",\"ssid\":\"%s\",\"lastPing\":%" PRIu64 ",\"temperature\":%d,\"version\":\"" PROJECT_VER "\", \"rssi\":%d, \"chip_model\":\"%s\",\"local_ping\":%d}",
             state.status, state.ssid, state.lastPing, state.temperature, state.rssi, get_chip_model(chip_info.model), (int)state.local_ping_ok);
