...
static const char *LOG_TAG = "md_wireless";
                                       ...
                                       int32_t event_id, void *event_data)
{
    if (event_base == SC_EVENT && event_id == SC_EVENT_GOT_SSID_PSWD)
    {
        ...
        md_config_set(md_config::GROUP, rvd_data);
        md_config_commit();
...
{
    const md_wireless_config_t *config = (md_wireless_config_t *)param;
    for (;;)
    {
...
static void enter_deep_sleep_wake_on_pin(gpio_num_t pin)
{
    esp_deepsleep_gpio_wake_up_mode_t mode = gpio_get_level(pin) ? ESP_GPIO_WAKEUP_GPIO_LOW : ESP_GPIO_WAKEUP_GPIO_HIGH;
    ESP_ERROR_CHECK(esp_deep_sleep_enable_gpio_wakeup(1 << pin, mode));
...
static void _smart_config_task(void *param)
{
    ...
    const esp_timer_create_args_t timeout_args = {
        .callback = [](void *arg)
        {
            const md_wireless_config_t *config = (md_wireless_config_t *)arg;
            if (gpio_get_level(config->sense_gpio))
            {
                ESP_LOGW(LOG_TAG, "Smart Config Timeout. Rebooting...");
                esp_restart();
            } else {
                ESP_LOGW(LOG_TAG, "Smart Config Timeout. Going to deep sleep.");
